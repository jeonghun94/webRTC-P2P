...
var webrtc = exports.webrtc = {
  disconnect: function disconnect() {
    ...
    for (var _i2 = 0, _Object$entries2 = Object.entries(peerConnections); _i2 < _Object$entries2.length; _i2++) {
      var _Object$entries2$_i = _slicedToArray(_Object$entries2[_i2], 2),
        _key = _Object$entries2$_i[0],
        connection = _Object$entries2$_i[1];
      connection.close();
    }
  },
  connect: function connect(room, userId, userType, callback) {
    socket = (0, _socket.io)("http://localhost:28099");
        ...
        var _ref = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee(user) {
          ...
          return _regeneratorRuntime().wrap(function _callee$(_context) {
            while (1) switch (_context.prev = _context.next) {
              ...
              case 2:
                pc = webrtc.createPeerConnection(user.id, user.userId, userId);
        ...
        return _regeneratorRuntime().wrap(function _callee2$(_context2) {
          while (1) switch (_context2.prev = _context2.next) {
            case 0:
              ...
              pc = webrtc.createPeerConnection(offerSendID, offerSendUserId, userId);
              if (pc && socket && socket.connected) {
                _context2.next = 4;
                break;
              }
            ...
            case 12:
              ...
              pc.ondatachannel = function (event) {
                ...
                event.channel.onclose = function (event) {
                  userList = userList.filter(function (r) {
                    return r.id !== offerSendID;
                  });
    ...
    socket.on("getAnswer", function (answer) {
      var sdp = answer.sdp,
        answerSendID = answer.answerSendID,
        answerSendUserId = answer.answerSendUserId;
      var pc = peerConnections[answerSendUserId];
      if (!pc) return;
    ...
    socket.on("user_exit", function (user) {
      ...
      userList = userList.filter(function (peerUser) {
        return peerUser.id !== user.id;
      });
    });
  },
  createPeerConnection: function createPeerConnection(id, remoteUserId, localUserId) {
    try {
  ...
  sendMessageAll: function sendMessageAll(message) {
    for (var _i3 = 0, _Object$entries3 = Object.entries(peerChannels); _i3 < _Object$entries3.length; _i3++) {
      var _Object$entries3$_i = _slicedToArray(_Object$entries3[_i3], 2),
        key = _Object$entries3$_i[0],
        channel = _Object$entries3$_i[1];
      try {
  ...
  sendMessage: function sendMessage(message, user) {
    var channel = peerChannels[user.userId];
    if (!channel) {
